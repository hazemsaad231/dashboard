...
export default function Services() {
  ...
  // دالة fetch قابلة لإعادة الاستخدام
  const fetchData = async () => {
    setLoading(true);
    try {
      // مؤقت: per_page كبير عشان نتأكد إن العنصر الجديد ييجي أثناء الديباغ
      const r = await axios.get(`${api}/${resource}?per_page=100`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
      const arr = Array.isArray(r.data) ? r.data : r.data?.data || [];
      console.log('services fetched:', arr);
      setData(arr);
      setAll(arr);
      // لو الكومبوننت فتح لأول مرة وجاي معاه newItem من صفحة الإضافة ضيفه فورًا
    } catch (e) {
      console.error(e);
  ...
  useEffect(() => {
    (async () => {
      ...
      // لو جاينا من صفحة add/update ومعاها newItem نضيفه فورًا (optimistic update)
      const newItem = (location as any).state?.newItem;
      if (newItem) {
        setData(prev => {
          const exists = prev.some(it => String(it.id ?? it._id) === String(newItem.id ?? newItem._id));
        ...
        // امسح الstate من الhistory عشان ميعادش لو رجعنا تاني
        try {
    ...
    // لو resource اتغير او الموقع اتغير نعمل refetch
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      setAll(prev => prev.filter(it => String(it.id) !== String(sel) && String(it._id) !== String(sel)));
    ...
    },
    {
      ...
      renderCell: (p: any) => (
        <div className="w-full h-full flex items-center justify-center">
          <Link
            to={`/services/${resource}/${p.id}`}
            className="cursor-pointer font-semibold text-slate-900 hover:text-blue-600 transition-colors truncate"
          >
            {p.value}
          </Link>
        </div>
    ...
    },
    {
      ...
      renderCell: (p: any) => (
        <div className="flex gap-3 justify-center w-full items-center h-full">
          ...
          <Link
            to={`/dashboard/addUser/${resource}/${p.id}`}
          ...
          <Link
            to={`/dashboard/services/${resource}/${p.id}`}
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <Link to="/dashboard/addUser/services">
          <button className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-medium transition-colors shadow-sm hover:shadow-md">
            + إضافة خدمة
          </button>
        </Link>
      </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-6 pb-6 border-b border-slate-200">
          <div className="relative w-full md:w-auto">
            ...
            <input
              ...
              className="w-full md:w-80 h-11 rounded-lg pl-4 pr-10 bg-slate-50 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all outline-none"
            />
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-3 pt-4 border-t border-slate-200">
           <div>
            <p className="text-sm text-slate-600">
              عرض {Math.min(lastIndex, data.length)} من أصل {data.length} خدمة
            </p>
           </div>
      ...
      <Dialog open={open} onClose={closeDelete} className="relative z-50">
        <div className="fixed inset-0 z-50 w-72 md:w-screen m-auto overflow-y-auto flex items-center justify-center">
          <DialogPanel className="w-full max-w-md rounded-xl bg-white px-6 py-8 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
