...
const Add_Update_Chance: React.FC = () => {
  ...
  // select change handler (multiple)
  const onCategoriesSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const opts = Array.from(e.target.selectedOptions);
    const ids = opts.map((o) => Number(o.value)).filter(Boolean);
    setSelectedCategoryIds(ids);
  };

  // helper: convert selected ids to formData entries
    ...
    <div className="lg:mr-52 min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 pt-16 pb-12 p-3">
      <div className="flex justify-center items-start">
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-xl w-full max-w-4xl rounded-2xl p-6 space-y-6">
          ...
          {/* ------------------ هنا استبدلنا الفورم بتاع التصنيفات بـ select ------------------ */}
          {/* <div>
            ...
            <div className="flex gap-3 items-start">
              <select
                ...
                className="w-full border rounded p-2 h-36 bg-white"
              >
                {loadingCategories ? (
                  ...
                  allCategories.map((c) => (
                    <option key={String(c.id ?? c.name)} value={String(c.id ?? "")}>
                      {c.name}
                    </option>
                  ))
                )}
            ...
            <p className="text-xs text-slate-500 mt-1">اضغط Ctrl (أو Cmd) لاختيار أكثر من تصنيف</p>
          </div> */}
          {/* بدل بلوك select القديم: */}
<Select
  categories={allCategories}
  loading={loadingCategories}
  selectedIds={selectedCategoryIds}
  onChange={(ids) => setSelectedCategoryIds(ids)}
  placeholder="اختر التصنيفات المفضلة"
/>
